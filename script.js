    ...
    this.callbacks = {
      ...
      leads: {
        selected: function () {
          ...
          var leadsQuery = selectedLeadsIds.map(function(id) {
            return 'filter[id][]=' + id
          }).join('&')
          ...
          $.get('/api/v4/leads?with=contacts&' + leadsQuery, function(data) {
            ...
            var contactsQuery = contactsIds.map(function(id) {
              return 'filter[id][]=' + id
            }).join('&')
            ...
            $.get('/api/v4/contacts?' + contactsQuery, function(data) {
              ...
              var companiesQuery = companiesIds.map(function(id) {
                return 'filter[id][]=' + id
              }).join('&')
              ...
              $.get('/api/v4/companies?' + companiesQuery, function(data) {
                ...
                var excelTable = [
                  ['Название', 'Дата создания', 'Теги', 'Кастомные поля', 'Компании', 'Контакты'],
                ];
                
                selectedLeads.forEach(lead => {
                  //== Формирую дату создания сделки в формате DD.MM.YYYY
                  var creationDate = new Date(lead.created_at)
                  var beautifyCreationDate = creationDate.getDate() + '.' + creationDate.getMonth() + '.' + creationDate.getFullYear()
                  ...
                  var companiesNames = companies.reduce(function(memo, company) {
                    var isCompanySelected = selectedCompanies.find(function(c) {
                  ...
                  var contactsNames = contacts.reduce(function(memo, contact) {
                    var isContactSelected = selectedContacts.find(function(c) {
                  ...
                  var tags = lead._embedded.tags.map(function(tag) { return tag.name }).join(', ')

                  excelTable.push([lead.name, beautifyCreationDate, tags, ' ', companiesNames, contactsNames ])
                });
